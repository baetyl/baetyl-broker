...
func (c *ClientMQTT) onConnect(p *mqtt.Connect) error {
	...
	if p.Will != nil {
		if !common.CheckTopic(p.Will.Topic, c.getSysTopics(), false) {
...
func (c *ClientMQTT) onPublish(p *mqtt.Publish) error {
	...
	if !common.CheckTopic(p.Message.Topic, c.getSysTopics(), false) {
...
func (c *ClientMQTT) genSuback(p *mqtt.Subscribe) (*mqtt.Suback, []mqtt.Subscription) {
	...
	for i, sub := range p.Subscriptions {
		if !common.CheckTopic(sub.Topic, c.getSysTopics(), true) {
